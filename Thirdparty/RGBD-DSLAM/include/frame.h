...
public:
  ...
  cv::Mat                   depth_; //!<depth image of the frame.
  double                    depth_scale_;
  Frame(vk::AbstractCamera* cam, const cv::Mat& img, const cv::Mat depth_image,const double depth_scale, double timestamp);
  ~Frame();
  ...
  /// Transforms pixel coordinates (c) to frame unit sphere coordinates (f).归一化坐标，模为1
  ...
  /// (x,y), (cols,rows)
  inline Vector3d proj2dTo3d(const double x, const double y) const
  {
    Vector3d unit_3d = cam_->cam2world(x,y);
    Vector3d norm_3d = unit_3d/unit_3d[2];
    ushort d = depth_.ptr<ushort>(int(y)) [int(x)];
    //if( (0.4*Config::depthScale()) < d && d < (5*Config::depthScale())) 
    if( (0.4*depth_scale_) < d && d < (5*depth_scale_)) 
      return norm_3d*float(d)/depth_scale_;
    else
      return Vector3d(0.0,0.0,0.0);
      
  }
   inline Vector3d proj2dTo3d(const Vector2d& px) const
   {
     return proj2dTo3d(px[0],px[1]);
  }
