        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //用于构建2d 地图 --@rongbohou
    mMapping2d = new Mapping2d(); //ATTENTION 
...
//add scan data -@rongbo.
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp,const sensor_msgs::LaserScanConstPtr &scan)
{
    if(mSensor!=RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    return mpTracker->GrabImageRGBD(im,depthmap,timestamp,scan);
}
...
// get the scan data of last key frame -@rongbohou
sensor_msgs::LaserScan System::GetLastKeyframeScan()
{
   vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
   sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
   KeyFrame* LastKeyframe = vpKFs[vpKFs.size()-1];
   //如果没有激光数据,返回一个空的
   if(LastKeyframe->mscan == NULL)
   {
     std::cout<<" have no scan data"<<std::endl;
     sensor_msgs::LaserScan scan;
     unsigned int num_readings = 2;
     int laser_frequency = 2;
   ...
   return *(LastKeyframe->mscan); //需要构造keyframe
}
...
void System::PublishMap()
{
   ...
   vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
   sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
   mMapping2d->MainMapping(vpKFs);
   

}
//get the nunber of current frame keypoint
int System::GetCFKeypointNum()
  {
